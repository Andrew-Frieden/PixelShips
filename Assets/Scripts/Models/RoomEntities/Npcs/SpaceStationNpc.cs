...
public class SpaceStationNpc : FlexEntity
{
    ...
    {
        return "A slowly rotating <> orbits a nearby system.".Encode(Name, Id, LinkColors.NPC);
    }
    ...
    private class RepairAction : SimpleAction
    {
        ...
        public override IEnumerable<string> Execute(IRoom room)
        {
            ...
            if (ship.Stats[StatKeys.Resourcium] >= Cost)
            {
                ...
                return new string[] { $"Your ship is repaired {repairAmount} hull." };
            }
            else
            {
                return new string[] { "You try to make a deal but turn up empty handed." };
            }
        }
    }
    ...
    private class TradeCommoditiesAction : SimpleAction
    {
        ...
        public override IEnumerable<string> Execute(IRoom room)
        {
            ...
            return new string[] { $"<> make a deal for {Profit} resourcium".Encode(Source.GetLinkText(), Source.Id, LinkColors.Player) };
        }
    }
}
